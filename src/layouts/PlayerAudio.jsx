...
export const PlayerAudio = () => {
  ...
  useEffect(() => {
    if (audioList) {
      setAudioSrc(audioList[audioIndex]);
    }
  ...
  return (
    <>
      {audioSrc && (
        <div className="fixed bottom-2 right-3 left-3 bg-zinc-200 dark:bg-zinc-900 border-2 border-black/10 dark:border-white/10 rounded-md p-2">
          <div className="controls flex justify-around items-center">
            {audioSrc && <p>{audioSrc[0]}</p>}
            <button
              onClick={() => setAudioIndex((prev) => prev - 1)}
              className="next w-6 h-6"
            >
              <svg
                version="1.1"
                id="Layer_1"
            ...
            <button
              ...
              className={`w-6 h-6 ${audioControls === null && "opacity-60"}`}
            >
